...
@calculate_time
def Talker_response(method = 'SadTalker', text = '', voice = 'zh-CN-XiaoxiaoNeural', rate = 0, volume = 100, pitch = 0, batch_size = 2):
    ...
    if method == 'SadTalker':
        ...
        video = sadtalker.test(source_image,
                            driven_audio,
                            preprocess_type,
                            is_still_mode,
                            enhancer,
...
def main():
    with gr.Blocks(analytics_enabled=False, title = 'Linly-Talker') as inference:
        ...
        with gr.Row(equal_height=False):
            ...
            video_button.click(fn=Talker_response,inputs=[method, input_text, voice, rate, volume, pitch, batch_size],outputs=[gen_video])
