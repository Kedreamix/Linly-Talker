...
@calculate_time
def Talker_response(method = 'SadTalker', text = '', voice = 'zh-CN-XiaoxiaoNeural', rate = 0, volume = 100, pitch = 0, batch_size = 2, character = '女性角色'):
    if character == '女性角色':
        # 女性角色
...
@calculate_time
def Talker_response_img(method, text, voice, rate, volume, pitch, source_image,
                    preprocess_type, 
                    is_still_mode,
                    enhancer,
                    batch_size,                            
...
def app():
    with gr.Blocks(analytics_enabled=False, title = 'Linly-Talker') as inference:
        ...
        with gr.Row(equal_height=False):
                ...
                with gr.Tabs(elem_id="question_audio"):
                    with gr.TabItem('对话'):
                        with gr.Column(variant='panel'):
                            ...
                            voice, rate, volume, pitch, batch_size, character, method = voice_setting()
            ...
            video_button.click(fn=Talker_response,inputs=[method, input_text,voice, rate, volume, pitch, batch_size, character],outputs=[gen_video])
                ...
                with gr.Tabs(elem_id="question_audio"):
                    with gr.TabItem('对话'):
                        with gr.Column(variant='panel'):
                            question_audio = gr.Audio(sources=['microphone','upload'], type="filepath", label = '语音对话')
                            input_text = gr.Textbox(label="Input Text", lines=3)
                            voice, rate, volume, pitch, batch_size = voice_setting()
                            asr_text = gr.Button('语音识别（语音对话后点击）')
                            asr_text.click(fn=Asr,inputs=[question_audio],outputs=[input_text])
            with gr.Column(variant='panel'): 
                with gr.Tabs():
